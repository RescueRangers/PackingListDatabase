...
SELECT A.[PacklisteId]
  ...
  WHERE A.PacklisteNumber = @PacklisteNumber";
        ...
        public async Task<bool> Insert(Packliste packliste)
        {
            ...
            using (var db = Connection)
            {
                const string sql = @"INSERT INTO [Packlistes] (Destination, PacklisteDate, PacklisteNumber) VALUES (@Destination, @PacklisteDate, @PacklisteNumber) SELECT CAST(scope_identity() AS int);";
                const string itemsSql = @"INSERT INTO [ItemWithQties] ([Quantity], [Item_ItemId], [Packliste_PacklisteId]) VALUES (@Quantity, @Item_ItemId, @Packliste_PacklisteId)";
                const string rawUsageSql = @"
     INSERT INTO [dbo].[MaterialAmounts]
           ([Amount]
           ,[Material_MaterialId]
           ,[Packliste_PacklisteId])
     VALUES
           (@Amount
           ,@Material_MaterialId
           ,@Packliste_PacklisteId)";
                const string dataSql = @"INSERT INTO [dbo].[PacklisteDatas] ([RowNumber], [ColumnNumber], [Data], [Packliste_PacklisteId]) VALUES (@RowNumber, @ColumnNumber, @Data, @Packliste_PacklisteId)";

                var id = await db.ExecuteScalarAsync<int>(sql, new { packliste.Destination, packliste.PacklisteDate, packliste.PacklisteNumber }).ConfigureAwait(false);

                if (id == 0) return false;

                foreach (var item in packliste.ItemsWithQties)
                {
                    await db.ExecuteAsync(itemsSql, new { Item_ItemId = item.Item.ItemId, Quantity = item.Quantity, Packliste_PacklisteId = id }).ConfigureAwait(false);
                }

                foreach (var usage in packliste.RawUsage)
                {
                    await db.ExecuteAsync(rawUsageSql, new { Amount = usage.Amount, Material_MaterialId = usage.MaterialId, Packliste_PacklisteId = id});
                }

                foreach (var data in packliste.PacklisteData)
                {
                    await db.ExecuteAsync(dataSql, new { RowNumber = data.RowNumber, ColumnNumber = data.ColumnNumber, Data = data.Data, Packliste_PacklisteId = id }).ConfigureAwait(false);
                }
                return true;
            }
        }
