    ...
    public class ImportsRepository : RepositoryBase, IImportsRepository
    {
        ...
        public async Task<bool> Insert(ImportTransport importtransport)
        {
            ...
            using (var db = Connection)
            {
                ...
                var result = await db.ExecuteAsync(sql, new { importtransport.Sender, importtransport.ImportDate }).ConfigureAwait(false);
        ...
        public async Task<bool> Update(int id, ImportTransport importtransport)
        {
            ...
            using (var db = Connection)
            {
                ...
                var result = await db.ExecuteAsync(sql, new { importtransport.Sender, importtransport.ImportDate, ImportTransportId = id });
